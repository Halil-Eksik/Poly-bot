...
| Area | Status |
|------|--------|
| CLOB SDK | `@polymarket/clob-client-v2` (TypeScript) / `py-clob-client-v2` (Python legacy) |
...
```bash
npm install
# uses @polymarket/clob-client-v2 from package.json
```
...
- [ ] `@polymarket/clob-client-v2` installed (`npm install`) **or** `py-clob-client-v2` (Python legacy)
